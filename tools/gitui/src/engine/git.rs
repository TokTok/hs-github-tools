...
pub trait Git: Send + Sync {
    ...
    fn check_conflict(&self, branch_name: &str, new_parent_name: &str) -> anyhow::Result<bool>;
    fn check_conflict_between(&self, oid_a: Oid, oid_b: Oid) -> anyhow::Result<bool>;
        ...
        if let Ok(mut walk) = repo.revwalk() {
            ...
            for commit_oid in walk.flatten() {
                    ...
                    && other_branch != branch_name
                {
                    ...
                    if Some(&commit_oid) != other_oid || commit_oid != oid {
...
impl Git for RealGit {
    ...
    fn check_conflict(&self, branch_name: &str, new_parent_name: &str) -> anyhow::Result<bool> {
        ...
        let parent_oid = repo
            ...
            .id();
        drop(repo);
        self.check_conflict_between(branch_oid, parent_oid)
    }

    fn check_conflict_between(&self, branch_oid: Oid, parent_oid: Oid) -> anyhow::Result<bool> {
        let repo = self.repo()?;
        let our_commit = repo.find_commit(branch_oid)?;
        let their_commit = repo.find_commit(parent_oid)?;

        let index = repo.merge_commits(&their_commit, &our_commit, None)?;
