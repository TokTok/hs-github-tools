    ...
    loop {
        ...
        while let Ok(resp) = resp_rx.try_recv() {
            let msg = match resp {
                ...
                GitResponse::ConflictCheck(branch, onto, base, res) => {
                    Msg::ConflictChecked(branch, onto, base, res)
                }
...
async fn handle_effects(
    ...
    for effect in effects {
        match effect {
            ...
            Effect::CheckConflict { branch, onto, base } => {
                cmd_tx
                    .send(GitCommand::CheckConflict { branch, onto, base })
                    .await?;
            }
