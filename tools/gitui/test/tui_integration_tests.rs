...
#[tokio::test]
async fn test_tui_divergence_visibility() {
    let (dir, repo, _master) = setup_repo();
    let path_str = dir.path().to_str().unwrap();
    ...
    // Now branch-b should be diverged from branch-a because branch-a moved.

    let git = RealGit::new(dir.path()).unwrap();
    let (branches, history) = git.get_branches(None).unwrap();

    let mut state = AppState {
        branches,
        history,
        initial_branch: "branch-b".to_string(),
        ..Default::default()
    };
    state.refresh_tree(None);
    ...
    terminal
        .draw(|f| ui::main_view::draw_main(f, &mut state))
        .unwrap();

    let output = buffer_to_string(terminal.backend().buffer());
    println!("{}", output);
    assert!(
        output.contains("branch-b [DIVERGED]"),
        "TUI should show [DIVERGED] for branch-b"
    );
}

#[tokio::test]
async fn test_tui_divergence_visibility_different_summary() {
    let (dir, repo, _master) = setup_repo();
    let path_str = dir.path().to_str().unwrap();
    ...
    // 4. Amend Branch A with DIFFERENT summary
    run_git(path_str, &["checkout", "branch-a"]);
    let file1_path = dir.path().join("file1.txt");
    std::fs::write(&file1_path, "content1 updated").unwrap();
    run_git(path_str, &["add", "file1.txt"]);
    run_git(path_str, &["commit", "--amend", "-m", "Different Summary"]);
    ...
    assert!(
        output.contains("branch-b [DIVERGED]"),
        "TUI should show [DIVERGED] for branch-b even with different summary"
    );
}
