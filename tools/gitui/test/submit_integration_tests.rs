...
#[test]
fn test_submit_middle_branch_in_stack() {
    let (dir, repo, initial_branch) = setup_repo();
    let path_str = dir.path().to_str().unwrap();
    ...
    // 2. Setup tree: master -> feat-a -> feat-b
    run_git(path_str, &["checkout", "-b", "feat-a", &base_oid]);
    create_commit(&repo, "a.txt", "a", "Commit A");

    run_git(path_str, &["checkout", "-b", "feat-b"]);
    create_commit(&repo, "b.txt", "b", "Commit B");
    let feat_b_oid = run_git(path_str, &["rev-parse", "HEAD"]);

    // Push feat-b to origin to make it submittable
    run_git(path_str, &["push", "origin", "feat-b"]);
    run_git(
        path_str,
        &["branch", "--set-upstream-to=origin/feat-b", "feat-b"],
    ...
    // 5. Verify results
    let master_oid = run_git(path_str, &["rev-parse", &master]);
    assert_eq!(master_oid, feat_b_oid);

    // Both feat-a and feat-b should be deleted as they are both merged into master
    let branches_after = run_git(path_str, &["branch"]);
    assert!(!branches_after.contains("feat-b"), "feat-b was not deleted");
    assert!(!branches_after.contains("feat-a"), "feat-a was not deleted");

    // Verify it was correctly pushed to upstream master
    let upstream_master_oid = run_git(path_str, &["ls-remote", "upstream", &master]);
    assert!(upstream_master_oid.contains(&feat_b_oid));
}
